...
for(let i = 0; i < miCamino.material.length; i++) {
  ...
  let materialNeeded = miCamino.material[i].nombre;
  let description = miCamino.material[i].descripcion;

  if (i === 0) {
    console.log(`Lo primero que debo llevar es ${materialNeeded} y es ${description}`);
  } else if (i === 1) {
    console.log(`lo segundo que debo llevar es ${materialNeeded} y es ${description}`);
  } else if (i === 2) {
    console.log(`lo tercero que debo llevar es ${materialNeeded} y es ${description}`);
  } else {
    console.log(`y por último debo llevar es ${materialNeeded} que es ${description}`);
  }
}

//4º: vamos a decir: para subir al {nombre de la cima}  debo subir {altura } metros y debo viajar a {ubicacion}

for(let tops of miCamino.cimas) {
  console.log(`para subir al ${tops.nombre}  debo subir ${tops.altitud} metros y debo viajar a ${tops.localizacion}`)
};
...
console.log(total/miCamino.cimas.length)
